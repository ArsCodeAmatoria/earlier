import Link from "next/link";
import { Scene16EarlySearch } from "@/components/screenplay/scene-16-early-search";
import { ScreenplayShell } from "@/components/screenplay/screenplay-shell";

export const metadata = {
  title: "Screenplay · Scene 16 — earlier",
  description:
    "EXT. forest, night — flashlight, ringtone that moves, ground that feels familiar; Nora denies every anchor.",
};

export default function Scene16Page() {
  return (
    <main className="pb-24 pt-[5.5rem] md:pt-28">
      <div className="site-container max-w-3xl">
        <div className="mb-10 border-b border-zinc-800 pb-8">
          <Link
            href="/script#beat-14"
            className="type-display-md text-[10px] text-zinc-500 transition-colors hover:text-zinc-300"
          >
            ← Structure outline · Beat 14
          </Link>
          <p className="type-display-md mt-4 text-[10px] text-zinc-600">
            Scene 16 · 133–146 · Finale (early)
          </p>
        </div>

        <ScreenplayShell>
          <Scene16EarlySearch />
        </ScreenplayShell>
      </div>
    </main>
  );
}
