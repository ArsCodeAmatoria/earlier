import Link from "next/link";
import { Scene02WalkInTrail } from "@/components/screenplay/scene-02-walk-in-trail";
import { ScreenplayShell } from "@/components/screenplay/screenplay-shell";

export const metadata = {
  title: "Screenplay · Scene 2 — earlier",
  description: "EXT. forest trail — Leanne, Maya, and Nora hike deeper into the woods.",
};

export default function Scene2Page() {
  return (
    <main className="pb-24 pt-[5.5rem] md:pt-28">
      <div className="site-container max-w-3xl">
        <div className="mb-10 border-b border-zinc-800 pb-8">
          <Link
            href="/script#beat-02"
            className="type-display-md text-[10px] text-zinc-500 transition-colors hover:text-zinc-300"
          >
            ← Structure outline · Beat 2
          </Link>
          <p className="type-display-md mt-4 text-[10px] text-zinc-600">
            Scene 2 · 3–4 · Theme Stated
          </p>
        </div>

        <ScreenplayShell>
          <Scene02WalkInTrail />
        </ScreenplayShell>
      </div>
    </main>
  );
}
